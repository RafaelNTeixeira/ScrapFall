# =============================================================================
# LevelTransitionUI.gd — CanvasLayer (layer=20)
# Handles the two-step level advance flow:
#   Step 1 — Confirmation panel
#   Step 2 — Buff picker (3 cards)
# Add as a child of UILayer in Main.tscn (or add via TabNavigator._ready).
# =============================================================================
extends CanvasLayer
...
# -----------------------------------------------------------------------------
# Public — called by ShippingUI when player taps "Advance Level"
# -----------------------------------------------------------------------------
...
func _build_confirm_panel() -> void:
	_confirm_panel = _make_overlay_panel()
	...
	var warn := Label.new()
	warn.text            = "⚠ All resources will reset to 0.\nGold, upgrades and buffs are kept."
...
func _refresh_confirm_text() -> void:
	...
	var layout_name: String = LevelManager.LAYOUTS[
		mini(next_level - 1, LevelManager.LAYOUTS.size() - 1)
	].get("name", "Unknown")

	var title: Label = _confirm_panel.get_node("VBoxContainer/Title")
	title.text = "Advance to Level %d?" % next_level

	var body: Label = _confirm_panel.get_node("VBoxContainer/Body")
...
func _build_buff_panel() -> void:
	_buff_panel = _make_overlay_panel()
	_buff_panel.visible = false
	...
	# Placeholder — cards are added in _show_buff_picker
...
func _show_buff_picker() -> void:
	...
	var cards_box: VBoxContainer = _buff_panel.get_node("VBoxContainer/CardsBox")
...
func _make_buff_card(buff_id: String) -> Control:
	...
	# Current accumulated stat (shows value before this pick)
	var count: int = BuffManager.stack_count(buff_id)
	var stat_lbl := Label.new()
	if count > 0:
		var current_val: float = BuffManager.stacked_value(buff_id)
		stat_lbl.text = "Current: +%.0f%s   →   After: +%.0f%s" % [
			current_val, def["stat_unit"],
			current_val + def["stat_per_stack"], def["stat_unit"]
		]
